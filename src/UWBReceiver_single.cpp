...
class UWBRcv_SINGLE : public rclcpp::Node {
public:
  UWBRcv_SINGLE() : Node("uwb_rcv_single")
  {
	// Define QoS
	qos_anc.best_effort();
    ...
    timer_ = this->create_wall_timer(1ms, std::bind(&UWBRcv_SINGLE::receive_loop, this));
  }

  ~UWBRcv_SINGLE() override { close(sockfd_); }
...
int main(int argc, char *argv[])
{
  ...
  rclcpp::spin(std::make_shared<UWBRcv_SINGLE>());
  rclcpp::shutdown();
  return 0;
}
